...
namespace OpenUtau.Audio {
    public class MiniAudioOutput : IAudioOutput, IDisposable {
        ...
        public static int DeviceChannels { get; private set; } = 2;
        ...
        public MiniAudioOutput() {
            ...
            if (Preferences.Default.UseSystemDefaultAudioDevice) {
                ...
                if (nativeContext != IntPtr.Zero) return;
                Log.Warning("Failed to init default audio device, falling back to enumerated.");
            }
            UpdateDeviceList();
            if (Guid.TryParse(Preferences.Default.PlaybackDevice, out var guid)) {
                ...
                foreach (AudioOutputDevice dev in devices) {
                    try { SelectDevice(dev.guid, dev.deviceNumber); foundDevice = true; break; } 
                    catch (Exception e) { Log.Warning(e, $"Failed to init audio device {dev}"); }
                }
                if (!foundDevice) throw new Exception("Failed to init any audio device");
            }
        }
        ...
        private void UpdateDeviceList() {
            ...
            unsafe {
                ...
                if (count == 0) throw new Exception("Failed to get any audio device info");
                ...
                for (int i = 0; i < count; i++) {
                    ...
                    string api = Marshal.PtrToStringUTF8(device_infos[i].api);
                    ...
                    devices.Add(new AudioOutputDevice {
                        name = name, api = api, deviceNumber = i, guid = new Guid(guidData),
                    });
                }
        ...
        public void Play() {
            if (PlaybackState != PlaybackState.Playing) CheckError(ou_audio_device_start(nativeContext));
            if (PlaybackState != PlaybackState.Paused) currentTimeMs = 0;
            PlaybackState = PlaybackState.Playing; eof = false;
        }

        public void Pause() {
            if (PlaybackState == PlaybackState.Playing) CheckError(ou_audio_device_stop(nativeContext));
        ...
        private unsafe void DataCallback(float* buffer, uint channels, uint frame_count) {
            ...
            if (temp.Length < samples) temp = new float[samples];
            
            int n = 0;
            if (sampleProvider != null) n = sampleProvider.Read(temp, 0, samples);
            if (n < samples) Array.Fill(temp, 0, n, samples - n);
            if (n == 0) eof = true;
        ...
        public long GetPosition() {
            if (eof && PlaybackState == PlaybackState.Playing) Stop();
            return (long)(Math.Max(0, currentTimeMs) / 1000 * sampleRate * 4);
        }

        public void SelectDevice(Guid guid, int deviceNumber) {
            if (Preferences.Default.UseSystemDefaultAudioDevice) return;
            if (selectedDevice != Guid.Empty && selectedDevice == guid) return;
            if (nativeContext != IntPtr.Zero) {
                CheckError(ou_free_audio_device(nativeContext));
                nativeContext = IntPtr.Zero; selectedDevice = Guid.Empty;
            }
            for (int i = 0; i < devices.Count; i++) {
                if (devices[i].guid == guid) { deviceNumber = i; break; }
                if (i == devices.Count - 1) { guid = devices[0].guid; deviceNumber = devices[0].deviceNumber; }
            }
            uint api_id; ulong id;
            unsafe {
                fixed (byte* guidPtr = guid.ToByteArray()) {
                    api_id = (uint)*(ulong*)guidPtr; id = *(ulong*)(guidPtr + 8);
                }
            }
            unsafe {
                nativeContext = ou_init_audio_device(api_id, id, callbackPtr);
                if (nativeContext == IntPtr.Zero) throw new Exception("Failed to init audio device");
            }
            selectedDevice = guid; DeviceNumber = deviceNumber;
        ...
        public List<AudioOutputDevice> GetOutputDevices() {
            if (Preferences.Default.UseSystemDefaultAudioDevice) return new List<AudioOutputDevice>();
            return devices;
        }
        ...
        private unsafe struct ou_audio_device_info_t {
            public IntPtr name; public ulong id; public IntPtr api; public uint api_id;
        }
        ...
        private static void CheckError(int errorCode) {
            ...
            throw new Exception(Marshal.PtrToStringUTF8(ou_audio_get_error_message(errorCode)));
        }
        #endregion
        ...
        protected virtual void Dispose(bool disposing) {
            if (!disposedValue) {
                if (nativeContext != IntPtr.Zero) {
                    ou_free_audio_device(nativeContext); nativeContext = IntPtr.Zero;
                }
                disposedValue = true;
            }
        }
        ~MiniAudioOutput() { Dispose(disposing: false); }
